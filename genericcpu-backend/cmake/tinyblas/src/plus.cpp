...
template<typename A, typename B, typename C>
FORCE_INLINE void tblas_tensor_geplus_broadcast_right(const A *a, const B *b, C *c, size_t na, size_t nb, size_t nc, size_t p) {
...
template<typename A, typename B, typename C>
FORCE_INLINE void tblas_tensor_geplus_broadcast_left(const A *a, const B *b, C *c, size_t na, size_t nb, size_t nc, size_t p) {
    for (size_t i = 0; i < nc; i++) {
        size_t aIdx = (i / p) % na;
        size_t bIdx = i % nb;
        c[i] = a[aIdx] + b[bIdx];
