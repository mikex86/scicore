...
FORCE_INLINE void tblas_tensor_gepow(A *a, B b, C *c, size_t n) {
    for (size_t i = 0; i < n; i++) {
        c[i] = pow(a[i], b);
    }
}
...
template<typename A, typename B>
FORCE_INLINE void tblas_tensor_gepow_broadcast_inplace(A *a, B *b, size_t n, size_t r) {
    for (size_t i = 0; i < n; i++) {
        size_t j = i % r;
        a[i] = pow(a[i], b[j]);
    }
...
template<typename A, typename B, typename C>
FORCE_INLINE void tblas_tensor_gepow_broadcast(A *a, B *b, C *c, size_t n, size_t r) {
    for (size_t i = 0; i < n; i++) {
        size_t j = i * r;
        c[i] = pow(a[i], b[j]);
    }
