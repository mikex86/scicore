...
class TokenizedBinaryTokenStreamer(private val sciCore: SciCore, tokenFile: Path, private val blockSize: Int, private val shuffle: Boolean = true):
    ...
    override fun get(): Pair<ITensor, ITensor> {
        if (shuffle)
            idx = random.nextLong(trainBin.length() / Short.SIZE_BYTES)
